...
<script type="text/javascript">
	...
	function deletee() {
		if (selectOne()) {
			formSubmit('${ctx}/fac/basicinfo/factory/delete', '_self');
		}
	}

	function chakan() {
		if (selectOne()) {
			formSubmit('${ctx}/fac/factory/Detail', '_self');
		}
	}

	function xiugai() {
		if (selectOne()) {
			formSubmit('${ctx}/fac/baseinfo/factory/update', '_self');
		}
	}
...
<body>
	<form name="form2" action="${ctx}/fac/list">
		<div id="menubar">
				...
				<div id="innerMenubar">
					<div id="navMenubar">
						<ul>
							...
							<li id="new"><a href="${ctx}/fac/basicinfo/factory/create">新建</a></li>
							<li id="update"><a href="#" onclick="xiugai();this.blur();">修改</a></li>
							<li id="delete"><a href="#" onclick="deletee();this.blur();">多个删除</a></li>
		...
		<div class="textbox" id="centerTextbox">
			...
			<div>
				...
				<div class="eXtremeTable">
					<table id="ec_table" border="1" cellspacing="0" cellpadding="0"
											...
											<td class="compactToolbar" align="right">
												<table border="0" cellpadding="1" cellspacing="2">
													<tr>
														...
														<td><img src="../images/table/separator.gif"
															style="border: 0" alt="Separator" /></td>
														<td><a href="${ctx}/fac/factory/exportExcel
														?pageSize=${pageNo*pageSize-((pageNo-1)*pageSize)}
														&pageNo=${(pageNo-1)*pageSize+1}&f_type=${f_type }"><img
						...
						<tbody class="tableBody">
							<c:forEach items="${dataList }" var="f" varStatus="status">
								...
								<td><input type="checkbox" name="id" dataType="复选框"
									dispName="厂家管理" minSelect="1" value="${f.FACTORY_ID }" /></td>
								<td>
								 <c:if test="${f.STATE==1}">显示</c:if>
										<c:if test="${f.STATE==0}">隐藏</c:if></td>
								<td><a href="${ctx}/fac/factory/Detail?id=${f.FACTORY_ID}">
										${f.FULL_NAME } </a></td>
										...
										<div id="middleMenubar">
											<div id="innerMenubar">
												<div id="navMenubar">
													<ul>
														<li id="view"><a
															href="${ctx}/fac/factory/Detail?id=${f.FACTORY_ID}">查看</a></li>
														<li id="update"><a
															href="${ctx}/fac/baseinfo/factory/update?id=${f.FACTORY_ID}">修改</a></li>
															<li id="delete"><a href="${ctx}/fac/basicinfo/factory/delete?id=${f.FACTORY_ID}" onclick="return del()">刪除</a></li>
													</ul>
												</div>
											</div>
										</div>
